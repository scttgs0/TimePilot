...
trackn_pause            * = *+TRACKS
trackn_note             * = *+TRACKS
trackn_volume           * = *+TRACKS
trackn_distor           * = *+TRACKS
trackn_shiftfrq         * = *+TRACKS

    .if FEAT_PORTAMENTO
trackn_portafrqc        * = *+TRACKS
trackn_portafrqa        * = *+TRACKS
trackn_portaspeed       * = *+TRACKS
trackn_portaspeeda      * = *+TRACKS
trackn_portadepth       * = *+TRACKS
    .endif

trackn_instrx2          * = *+TRACKS
trackn_instrdb          * = *+TRACKS
trackn_instrhb          * = *+TRACKS
trackn_instridx         * = *+TRACKS
trackn_instrlen         * = *+TRACKS
trackn_instrlop         * = *+TRACKS
trackn_instrreachend    * = *+TRACKS
trackn_volumeslidedepth * = *+TRACKS
trackn_volumeslidevalue * = *+TRACKS

    .if FEAT_VOLUMEMIN
trackn_volumemin        * = *+TRACKS
    .endif

FEAT_EFFECTS    = FEAT_EFFECTVIBRATO||FEAT_EFFECTFSHIFT

    .if FEAT_EFFECTS
trackn_effdelay         * = *+TRACKS
    .endif

    .if FEAT_EFFECTVIBRATO
trackn_effvibratoa      * = *+TRACKS
    .endif

    .if FEAT_EFFECTFSHIFT
trackn_effshift         * = *+TRACKS
    .endif

trackn_tabletypespeed   * = *+TRACKS

    .if FEAT_TABLEMODE
trackn_tablemode        * = *+TRACKS
    .endif

trackn_tablenote        * = *+TRACKS
trackn_tablea           * = *+TRACKS
trackn_tableend         * = *+TRACKS

    .if FEAT_TABLEGO
trackn_tablelop         * = *+TRACKS
    .endif

trackn_tablespeeda      * = *+TRACKS

    .if FEAT_FILTER||FEAT_BASS16
trackn_command          * = *+TRACKS
    .endif

    .if FEAT_BASS16
trackn_outnote          * = *+TRACKS
    .endif

    .if FEAT_FILTER
trackn_filter           * = *+TRACKS
    .endif
...
;--------------------------------------
;--------------------------------------
track_endvariables  * = PLAYER-$100-$140-$40+2
;--------------------------------------

INSTRPAR    = 12

tabbeganddistor .byte frqtabpure-frqtab,$00
...
vib0            .char 0
vib1            .char 1,-1,-1,1
vib2            .char 1,0,-1,-1,0,1
vib3            .char 1,1,0,-1,-1,-1,-1,0,1,1
...
;--------------------------------------
    ...
    .if FEAT_BASS16
frqtabbasslo    .byte $F2,$33,$96,$E2,$38,$8C,$00,$6A,$E8,$6A,$EF,$80,$08,$AE,$46,$E6
...
frqtab
;.assert [<frqtab]!=0    ;* frqtab must begin at the memory page bound! (i.e. $..00 address)

frqtabbass1     .byte $BF,$B6,$AA,$A1,$98,$8F,$89,$80,$F2,$E6,$DA,$CE,$BF,$B6,$AA,$A1
...
frqtabbass2     .byte $FF,$F1,$E4,$D8,$CA,$C0,$B5,$AB,$A2,$99,$8E,$87,$7F,$79,$73,$70
...
frqtabpure      .byte $F3,$E6,$D9,$CC,$C1,$B5,$AD,$A2,$99,$90,$88,$80,$79,$72,$6C,$66
    ...
    .if FEAT_BASS16
frqtabbasshi    .byte $0D,$0D,$0C,$0B,$0B,$0A,$0A,$09,$08,$08,$07,$07,$07,$06,$06,$05
...
volumetab       .byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
...
oo1b            lda trackn_db,x
                sta ns
                lda trackn_hb,x
                sta ns+1

oo1i            ldy trackn_idx,x
                inc trackn_idx,x
...
p2o3            jmp GetSongLineTrackLineInitOfNewSetInstrumentsOnlyRmtp3
go_ppnext       jmp ppnext

rmt_p3          lda #>frqtab
                sta nr+1
xtracks05sub1   ldx #TRACKS-1
pp1             lda trackn_instrhb,x
                beq go_ppnext
...
ei2c2           inc trackn_tablea,x
ei2a            lda trackn_instrdb,x
                sta nt
...
ei3             lda trackn_instrreachend,x
                bpl ei4
...
pps1            sbc trackn_portadepth,x
                bcc pps8

                cmp trackn_portafrqc,x
                bcs pps9

pps8            lda trackn_portafrqc,x
pps9            sta trackn_portafrqa,x
pp10            lda reg2
                and #$01
                beq pp11
...
xstysta01       sty $d218
                sta $d208
    .elsif STEREOMODE==0    ;* L1 L2 L3 L4
                ldy #$ff
v_audctl        = *-1
                lda trackn_audf+0
                ldx trackn_audc+0
                ...
                stx $d201+6
                sty $d208
    .elsif STEREOMODE==2    ;* L1 R2 R3 L4
                ldy #$ff
v_audctl        = *-1
                lda trackn_audf+0
                ldx trackn_audc+0
                ...
                sty $d218
                sty $d208
    .elsif STEREOMODE==3    ;* L1 L2 R3 R4
                ldy #$ff
v_audctl        = *-1
                lda trackn_audf+0
                ldx trackn_audc+0
